...
types=c('location vs price',"location vs population","house age vs price","distance_to_ocean vs price")



ui <- fluidPage(
  # Application title
  titlePanel("Choose the plot you want to see"),
  sidebarLayout(
  sidebarPanel(
    selectInput("type","Choose a type", choices = types,selected = types[3])
             ),
  mainPanel(
    plotOutput("plot")
      )
...
server <- function(input, output) {
  output$plot=renderPlot({
    plot(x=switch(which(types==input$type),p1,p2,p3,p4),y=5)
  })
}
